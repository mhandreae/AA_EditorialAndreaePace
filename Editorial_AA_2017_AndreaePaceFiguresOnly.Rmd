...
figure3 <- ggplot(data = armplot.df) +
  ...
  scale_x_continuous('Control Group Dose [mg]', breaks = c(0, 1, 2, 3, 4), labels = math_format(exp^.x),  sec.axis = sec_axis(~exp(.), breaks =c(1,10,20,40,60,100)), limits = c(0.25, 5.77)) +
  scale_y_continuous('Acetaminophen Group Dose [mg]',breaks = c(0, 1, 2, 3, 4), labels = math_format(exp^.x), sec.axis = sec_axis(~exp(.), breaks =c(1,10,20,40,60,100)), limits = c(-.105, 5.5)) +
