...
figure3 <- ggplot(data = armplot.df) +
  ...
  scale_x_continuous(labels = math_format(exp^.x), sec.axis = ~exp(.), limits = c(0.25, 5.77)) +
  scale_y_continuous(labels = math_format(exp^.x), sec.axis = ~exp(.), limits = c(-.105, 5.5)) +
...
# choose a simple background and render with x and y axis limits
Figure3Final <- figure3 + theme_bw() + xlim(0.25, 5.77) +ylim(-.105, 5.5)  
ggsave(Figure3Final, file = "Figs/Figure3Final.png", device ="png",
       scale = 1, width = 8, height = 4,dpi = 300, limitsize = TRUE)
ggsave(Figure3Final, file = "Figs/Figure3Final.pdf", device ="pdf",
       scale = 1, width = 8, height = 4,dpi = 300, limitsize = TRUE)
Figure3Final
```
